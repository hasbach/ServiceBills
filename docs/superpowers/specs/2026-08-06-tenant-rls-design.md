...
**Status: Deferred, not implemented (2026-08-06).** While scoping this, we found the actual live exposure the Supabase advisor was warning about: `anon`/`authenticated` had full SELECT/INSERT/UPDATE/DELETE grants on every table via Supabase's auto-generated REST API — a real, direct, unauthenticated path to every tenant's data that had nothing to do with RLS. That was fixed immediately by revoking those grants (`REVOKE ALL ... FROM anon, authenticated`, plus matching `ALTER DEFAULT PRIVILEGES` so it can't silently reappear on a future table) — confirmed closed, the grants query now returns zero rows. With that closed, what's left is only the app-layer defense-in-depth case below (a future bug in `tenant_query()` usage), which was judged not worth the engineering/rollout cost right now. Left here as a reference if that calculus changes later — the design doesn't need to be redone, just picked back up.
